...
import { fetchAsuncionWeather } from '../../services/weather';
...
export const WeatherWidget: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadWeather = async () => {
      try {
        setWeather(await fetchAsuncionWeather(controller.signal));
        setError(false);
      } catch {
  ...
  const rows = weather ? [
    { label: '🌡️ Temperature', value: `${weather.temperature.toFixed(1)}°C` },
    ...
    { label: '☁️ Conditions', value: weather.description, emphasized: true },
    { label: '🌧️ Precipitation', value: `${weather.precipitation.toFixed(1)} mm` },
  ] : [];
